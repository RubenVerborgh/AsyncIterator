// import { AsyncIterator, TransformOptions } from './asynciterator';
...
// class CompositeMapFilterTransform<T> extends AsyncIterator<T> {
...
//   }
//   read(): T | null {
//     const func = this.transforms.reduceRight((f, transform) => {
//       return transform.filter ?
//         (item: any) => transform.function(item) ? f(item) : null :
//         (item: any) => transform.function(f(item))
//     }, (e: any) => e)
...
//   }
//   syncTransform<D>(transform: (item: T) => Generator<D>): AsyncIterator<D> {
//     return new SyncTransformer<T, D>(this, transform);
//   }
// }

// class SyncTransformer<T, D> extends AsyncIterator<D> {
//   constructor(source: AsyncIterator<T>, transform: (item: T) => Generator<D>) {
//     super();
//     let transformation: Generator<D> | null;
//     source.on('readable', () => {
//       this.emit('readable');
...
//     }
//   }
//   map<K>(map: (item: D) => K): AsyncIterator<K> {
//     return new CompositeMapFilter(this).map(map);
//   }
//   filter(filter: (item: T) => boolean): AsyncIterator<T> {
//     return new CompositeMapFilter(this).filter(filter);
//   }
// }
